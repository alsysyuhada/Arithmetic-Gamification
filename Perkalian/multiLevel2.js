...
var frameSuccess = document.getElementById("success-mult2");
...
function openNextLevel(){
    ...
    xhr1.onreadystatechange = function () {
        if (xhr1.readyState === 4) {
            ...
            if (xhr1.status === 200) {
                console.log('Progres level2 perkalian berhasil dikirim');
            } else {
                console.log('Terjadi kesalahan saat mengirim progres dialog');
            }
        }
    };
    xhr1.send('progresLevel2Perkalian=');
}
...
function kirimNilai() {
    ...
    xhr1.onreadystatechange = function () {
        if (xhr1.readyState === 4) {
            ...
            console.log(xhr1.responseText);
            if (xhr1.status === 200) {
                console.log('Nilai skorLevel2Perkalian berhasil dikirim');
            } else {
                console.log('Terjadi kesalahan saat mengirim nilai skorLevel2Perkalian');
            }
        }
    };
            console.log(skorLevel2Perkalian);
    xhr1.send('skorLevel2Perkalian=' + encodeURIComponent(skorLevel2Perkalian));
}
...
window.onload = function(){
    ...
    //mengambil skor level3 perkalian dari database
	var xhr1 = new XMLHttpRequest();
	xhr1.open('GET', '../data/ambil_skor.php?ambilSkorLevel2Perkalian', true);
	xhr1.onreadystatechange = function(){
		if (xhr1.readyState === 4 && xhr1.status === 200) {
            var skorLevel2DariDatabase = parseInt(xhr1.responseText);
            skorLevel2Perkalian = skorLevel2DariDatabase;
        }
	};
    xhr1.send();
	waktuAwal = new Date().getTime();
...
function nilaiDiFrame(){
	...
	if (frameSuccess) {
	  ...
	  if (elemPiala){
	  	elemPiala.style.background = 'url("../gambar/pperkalian2.png") no-repeat center';
...
function ubahGambarPetunjuk(){
	if(petunjuk){
		...
		if (elem) {
			elem.style.backgroundImage = 'url("../gambar/petunjuk_multiLevel2.png")'
		}
	}
}