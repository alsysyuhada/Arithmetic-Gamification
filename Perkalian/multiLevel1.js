...
//buat soal random
function generateQA(){
	var x = generateRandomNumber(1, 5);
	var y = generateRandomNumber(1, 10);
	jawabanBenar = x * y;
...
//membuat pilihan salah
	for(i=1; i<3; i++){
		if(i != pilihanBenar){
			var pilihanSalah;
			do{
				pilihanSalah = (generateRandomNumber(1, 10)*generateRandomNumber(1, 10));
			}while (jawaban.indexOf(pilihanSalah)>-1 || pilihanSalah > 50)
...
function openNextLevel(){
    ...
    xhr1.onreadystatechange = function () {
        if (xhr1.readyState === 4) {
            ...
            if (xhr1.status === 200) {
                console.log('Progres level1 perkalian berhasil dikirim');
            } else {
                console.log('Terjadi kesalahan saat mengirim progres level');
            }
        }
    };
    xhr1.send('progresLevel1Perkalian=');
}
...
function kirimNilai() {
    ...
    xhr1.onreadystatechange = function () {
        if (xhr1.readyState === 4) {
            ...
            console.log(xhr1.responseText);
            if (xhr1.status === 200) {
                console.log('Nilai skorLevel1Perkalian berhasil dikirim');
            } else {
                console.log('Terjadi kesalahan saat mengirim nilai skorLevel1Perkalian');
            }
        }
    };
    xhr1.send('skorLevel1Perkalian=' + encodeURIComponent(skorLevel1Perkalian));
}
...
window.onload = function(){
    ...
    //mengambil skor level1 perkalian dari database
	var xhr1 = new XMLHttpRequest();
	xhr1.open('GET', '../data/ambil_skor.php?ambilSkorLevel1Perkalian', true);
	xhr1.onreadystatechange = function(){
		if (xhr1.readyState === 4 && xhr1.status === 200) {
            var skorLevel1DariDatabase = parseInt(xhr1.responseText);
            skorLevel1Perkalian = skorLevel1DariDatabase;
        }
	};
    xhr1.send();
    ubahGambarPetunjuk();
...
function nilaiDiFrame(){
	...
	if (frameSuccess) {
	  ...
	  if (elemPiala){
	  	elemPiala.style.background = 'url("../gambar/pperkalian1.png") no-repeat center';
...
function ubahGambarPetunjuk(){
	if(petunjuk){
		...
		if (elem) {
			elem.style.backgroundImage = 'url("../gambar/petunjuk_multiLevel1.png")'
		}
	}
}