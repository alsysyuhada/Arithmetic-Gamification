...
var frameSuccess = document.getElementById("success-div3");
var skorBermain = 0;
...
function kirimNilai() {
    ...
    xhr1.onreadystatechange = function () {
        if (xhr1.readyState === 4) {
            ...
            if (xhr1.status === 200) {
                console.log('Nilai skorLevel3Pembagian berhasil dikirim');
            } else {
                console.log('Terjadi kesalahan saat mengirim nilai skorLevel3Pembagian');
            }
        }
    };
            console.log(skorLevel3Pembagian);
    xhr1.send('skorLevel3Pembagian=' + encodeURIComponent(skorLevel3Pembagian));
}
...
window.onload = function(){
	...
	var xhr1 = new XMLHttpRequest();
	xhr1.open('GET', '../data/ambil_skor.php?ambilSkorLevel3Pembagian', true);
	xhr1.onreadystatechange = function(){
		if (xhr1.readyState === 4 && xhr1.status === 200) {
            var skorLevel3DariDatabase = parseInt(xhr1.responseText);
            skorLevel3Pembagian = skorLevel3DariDatabase;
            console.log(skorLevel3Pembagian);
        }
	};
    xhr1.send();
...
function openNextLevel(){
    ...
    xhr1.onreadystatechange = function () {
        if (xhr1.readyState === 4) {
            ...
            console.log(xhr1.responseText);
            if (xhr1.status === 200) {
                console.log('Progres level3 Pembagian berhasil dikirim');
            } else {
                console.log('Terjadi kesalahan saat mengirim progres dialog');
            }
        }
    };
    xhr1.send('progresLevel3Pembagian=');
}
...
function nilaiDiFrame(){
	...
	if (frameSuccess) {
	  ...
	  if (elemPiala){
	  	elemPiala.style.background = 'url("../gambar/ppembagian3.png") no-repeat center';
...
function ubahGambarPetunjuk(){
	if(petunjuk){
		...
		if (elem) {
			elem.style.backgroundImage = 'url("../gambar/petunjuk_divLevel3.png")'
		}
	}
}