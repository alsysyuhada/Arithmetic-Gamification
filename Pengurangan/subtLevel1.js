...
//buat soal random
function generateQA(){
	var x = generateRandomNumber(1, 15);
	var y = generateRandomNumber(1, 5);

	while (x<=y){
		x = generateRandomNumber(1, 15);
...
function openNextLevel(){
    ...
    xhr1.onreadystatechange = function () {
        if (xhr1.readyState === 4) {
            ...
            if (xhr1.status === 200) {
                console.log('Progres level1 pengurangan berhasil dikirim');
            } else {
                console.log('Terjadi kesalahan saat mengirim progres level');
            }
        }
    };
    xhr1.send('progresLevel1Pengurangan=');
}
...
function kirimNilai() {
    ...
    xhr1.onreadystatechange = function () {
        if (xhr1.readyState === 4) {
            ...
            console.log(xhr1.responseText);
            if (xhr1.status === 200) {
                console.log('Nilai skorLevel1Pengurangan berhasil dikirim');
            } else {
                console.log('Terjadi kesalahan saat mengirim nilai skorLevel1Pengurangan');
            }
        }
    };
    xhr1.send('skorLevel1Pengurangan=' + encodeURIComponent(skorLevel1Pengurangan));
}
...
window.onload = function(){
    ...
    //mengambil skor level1 pengurangan dari database
	var xhr1 = new XMLHttpRequest();
	xhr1.open('GET', '../data/ambil_skor.php?ambilSkorLevel1Pengurangan', true);
	xhr1.onreadystatechange = function(){
		if (xhr1.readyState === 4 && xhr1.status === 200) {
            var skorLevel1DariDatabase = parseInt(xhr1.responseText);
            skorLevel1Pengurangan = skorLevel1DariDatabase;
        }
	};
    xhr1.send();
    ubahGambarPetunjuk();
...
function nilaiDiFrame(){
	...
	if (frameSuccess) {
	  ...
	  if (elemPiala){
	  	elemPiala.style.background = 'url("../gambar/ppengurangan1.png") center center / 95% no-repeat';
	  }
	}
}
...
function ubahGambarPetunjuk(){
	if(petunjuk){
		...
		if (elem) {
			elem.style.backgroundImage = 'url("../gambar/petunjuk_subtLevel1.png")'
		}
	}
}